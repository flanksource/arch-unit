...
// GetASTCache returns the global AST cache singleton
func GetASTCache() (*ASTCache, error) {
	var err error
	astCacheOnce.Do(func() {
		astCacheInstance, err = NewASTCache()
	})
	return astCacheInstance, err
}

// ResetASTCache resets the singleton (for testing)
func ResetASTCache() {
	astCacheMutex.Lock()
...
func (c *ASTCache) init() error {
	...
	// Dependency tables schema
	dependencySchema := `
	-- Dependencies table for storing project dependencies
	CREATE TABLE IF NOT EXISTS dependencies (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		name TEXT NOT NULL,
		type TEXT NOT NULL, -- 'go', 'npm', 'pip', 'helm', 'docker', 'stdlib'
		language TEXT,
		version TEXT,
		git_url TEXT,
		packages_json TEXT, -- JSON array of packages
		file_path TEXT, -- Source file (go.mod, package.json, etc.)
		file_hash TEXT,
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		last_modified TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		UNIQUE(name, type, version, file_path)
	);

	-- Relationships between AST nodes and dependencies
	CREATE TABLE IF NOT EXISTS dependency_relationships (
		id INTEGER PRIMARY KEY AUTOINCREMENT,
		ast_id INTEGER,
		dependency_id INTEGER NOT NULL,
		line_no INTEGER,
		text TEXT, -- Import statement or usage text
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		FOREIGN KEY (ast_id) REFERENCES ast_nodes(id) ON DELETE CASCADE,
	...
	CREATE INDEX IF NOT EXISTS idx_dependencies_language ON dependencies(language);
	...
	CREATE INDEX IF NOT EXISTS idx_dependency_relationships_ast ON dependency_relationships(ast_id);
	CREATE INDEX IF NOT EXISTS idx_dependency_relationships_dep ON dependency_relationships(dependency_id);
	`

	// Dependency aliases table for Git URL resolution caching
	aliasSchema := `
	...
	aliasIndexes := `
	CREATE INDEX IF NOT EXISTS idx_dependency_aliases_lookup ON dependency_aliases(package_name, package_type);
	CREATE INDEX IF NOT EXISTS idx_dependency_aliases_checked ON dependency_aliases(last_checked);
	`
	...
	if _, err := c.db.Exec(dependencySchema); err != nil {
		return fmt.Errorf("failed to create dependency schema: %w", err)
	}

	if _, err := c.db.Exec(dependencyIndexes); err != nil {
		return fmt.Errorf("failed to create dependency indexes: %w", err)
	}

	if _, err := c.db.Exec(aliasSchema); err != nil {
		return fmt.Errorf("failed to create alias schema: %w", err)
	}

	if _, err := c.db.Exec(aliasIndexes); err != nil {
		return fmt.Errorf("failed to create alias indexes: %w", err)
	}
...
// GetLibraryNode retrieves a library node by ID
func (c *ASTCache) GetLibraryNode(id int64) (*models.LibraryNode, error) {
	query := `
		SELECT id, package, class, method, field, node_type, language
		FROM library_nodes
		WHERE id = ?
	`
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get library node: %w", err)
	}
...
func (c *ASTCache) StoreASTNode(node *models.ASTNode) (int64, error) {
	...
	result, err := c.db.Exec(`
		...
		node.CyclomaticComplexity, len(node.Parameters), len(node.ReturnValues),
...
// StoreDependency stores a dependency in the cache
func (c *ASTCache) StoreDependency(dep *models.Dependency) (int64, error) {
	packagesJSON := ""
	if len(dep.Package) > 0 {
		data, err := json.Marshal(dep.Package)
		if err != nil {
	...
	result, err := c.db.Exec(`
		...
		(name, type, language, version, git_url, packages_json, file_path, file_hash, last_modified)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
		dep.Name, dep.Type, "", dep.Version, dep.Git, packagesJSON, "", "")
	
	if err != nil {
		return 0, fmt.Errorf("failed to store dependency: %w", err)
	}
	
	return result.LastInsertId()
}

// StoreDependencies stores multiple dependencies for a file
func (c *ASTCache) StoreDependencies(filepath string, deps []*models.Dependency) error {
	if len(deps) == 0 {
		return nil
	}
	...
	_, err = tx.Exec("DELETE FROM dependencies WHERE file_path = ?", filepath)
	if err != nil {
	...
	for _, dep := range deps {
		...
		_, err = tx.Exec(`
			...
			(name, type, language, version, git_url, packages_json, file_path, file_hash, last_modified)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
			dep.Name, dep.Type, "", dep.Version, dep.Git, packagesJSON, filepath, fileHash)
		
		if err != nil {
			return fmt.Errorf("failed to store dependency %s: %w", dep.Name, err)
		}
	}
...
// GetDependenciesForFile retrieves dependencies for a specific file
func (c *ASTCache) GetDependenciesForFile(filepath string) ([]*models.Dependency, error) {
	rows, err := c.db.Query(`
		SELECT id, name, type, language, version, git_url, packages_json
		FROM dependencies
		WHERE file_path = ?
		ORDER BY name`,
		filepath)
	...
	for rows.Next() {
		...
		var packagesJSON sql.NullString
		var id int64
		
		var language string
		err := rows.Scan(&id, &dep.Name, &dep.Type, &language, 
			&dep.Version, &dep.Git, &packagesJSON)
		if err != nil {
		...
		if packagesJSON.Valid && packagesJSON.String != "" {
			err = json.Unmarshal([]byte(packagesJSON.String), &dep.Package)
			if err != nil {
...
// GetAllDependencies retrieves all dependencies in the cache
func (c *ASTCache) GetAllDependencies() ([]*models.Dependency, error) {
	rows, err := c.db.Query(`
		SELECT DISTINCT id, name, type, language, version, git_url, packages_json
		FROM dependencies
		ORDER BY type, name`)
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &dep.Name, &dep.Type, &dep.Language, 
			&dep.Version, &dep.Git, &packagesJSON)
		if err != nil {
			return nil, fmt.Errorf("failed to scan dependency: %w", err)
		}
...
// StoreDependencyRelationship stores a relationship between an AST node and a dependency
func (c *ASTCache) StoreDependencyRelationship(rel *models.DependencyRelationship) error {
	_, err := c.db.Exec(`
		INSERT INTO dependency_relationships
		(ast_id, dependency_id, line_no)
		VALUES (?, ?, ?)`,
		rel.ASTID, rel.DependencyID, rel.LineNo)
	
	if err != nil {
		return fmt.Errorf("failed to store dependency relationship: %w", err)
	}
	
	return nil
}

// GetDependencyRelationships retrieves relationships for a dependency
func (c *ASTCache) GetDependencyRelationships(dependencyID int64) ([]*models.DependencyRelationship, error) {
	rows, err := c.db.Query(`
		SELECT id, ast_id, dependency_id, line_no
	...
	var relationships []*models.DependencyRelationship
	for rows.Next() {
		rel := &models.DependencyRelationship{}
		err := rows.Scan(&rel.ID, &rel.ASTID, &rel.DependencyID, &rel.LineNo)
		if err != nil {
...
func (c *ASTCache) GetDependencyAlias(packageName, packageType string) (*models.DependencyAlias, error) {
	query := `
		SELECT id, package_name, package_type, git_url, last_checked, created_at 
		FROM dependency_aliases 
		WHERE package_name = ? AND package_type = ?
	`
	
	row := c.db.QueryRow(query, packageName, packageType)
	
	alias := &models.DependencyAlias{}
	err := row.Scan(&alias.ID, &alias.PackageName, &alias.PackageType, 
		&alias.GitURL, &alias.LastChecked, &alias.CreatedAt)
...
func (c *ASTCache) StoreDependencyAlias(alias *models.DependencyAlias) error {
	...
	_, err := c.db.Exec(query, alias.PackageName, alias.PackageType, 
		alias.GitURL, alias.LastChecked, alias.CreatedAt)
