...
func (dm *DepthManager) ScanWithDepth(ctx *ScanContext, rootPath string) (*git.DependencyTree, error) {
...
func (dm *DepthManager) scanCurrentLevel(ctx *ScanContext, job git.ScanJob) ([]*models.Dependency, error) {
	if job.IsLocal {
	...
	worktreeCtx := NewScanContext(ctx.Task, worktreePath)
