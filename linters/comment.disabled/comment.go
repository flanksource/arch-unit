...
// serializeCommentFix serializes a comment fix for storage in violation
func (c *CommentAnalysisLinter) serializeCommentFix(fix *CommentFix) string {
	if fixData, err := json.Marshal(fix); err == nil {
		return string(fixData)
	}
	return ""
}
